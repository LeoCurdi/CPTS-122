...
int main(void) {
	...
	int menuSelection = 0;
	top:
	// start with menu option to view test results or run the program
	do {
		printf("Select from the following choices by entering a number (1-2):\n\t1.  view test results\n\t2.  run the program\n\n");
		scanf("%d", &menuSelection);
		system("cls");
		if (menuSelection < 1 || menuSelection > 2) printf("\t!! Enter a valid option (1-2)\n\n");
	...
	Node* pHead = NULL; // initialize the head pointer for the list
	menuSelection = 0;
	int isLoaded = 0, numSongsPrinted;
	while (menuSelection != 11 && menuSelection != 12) {
		...
		case 1: // load
			loadList(&pHead); // reads in all the data from the file and puts together the linked list
			isLoaded = 1;
		...
		case 4: // insert
			...
			break;
		case 5: // delete
			if (pHead != NULL) { // only call the function if theres actually songs in the list, delete will break if you call it with an empty list
				delete(&pHead);
			}
			else {
				system("cls");
